...
#Log methods
#----------------------------------------
GAME.LOGGER.can_log = (type)->
    #This function takes in a type (e.g., 'warn') and checks to see if
    #it exists in the GAME.LOGGER.options.log_level option
...
#Log() function
#----------------------------------------
GAME.LOGGER.log = (type)->
    #When using logger.log, you must pass in a type
    #    e.g.
    #        GAME.LOGGER.debug( arg1, arg2, etc. )
    ...
    if not GAME.LOGGER.can_log(type)
        return false
...
GAME.LOGGER.options.log_types = [
    'debug', 'error', 'info', 'warn'
]

GAME.LOGGER.options.setup_log_types = ()->
    #This function will setup log types based on the ones available in
    #GAME.LOGGER.options.log_types.  It can be called whenever to 
    #dynamically add log types
    GAME.LOGGER.log('logger', 'setupt_log_types()',
        'Called setup log types!')
    
    for log_type in GAME.LOGGER.options.log_types
        #Wrap this in a closure
        do (log_type) ->
            #Create a logger[log_type] function for each different log type
            GAME.LOGGER[log_type] = ()->
                #Store all arguments
                args = Array.prototype.slice.call(arguments)
