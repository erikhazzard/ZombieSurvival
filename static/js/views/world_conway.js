  ...
  define(["lib/backbone", "models/cell"], function(Backbone, Cell) {
    ...
    World = (function(_super) {
      ...
      World.prototype.evolveCell = function(cell) {
        ...
        state = 'dead';
        if ((cell.get('state') === 'alive') && (this.model.get('rules').stayAlive.indexOf(neighbors.alive) > -1)) {
          state = 'alive';
          if (Math.random() < 0.005) state = 'zombie';
        } else if (this.model.get('rules').birth.indexOf(neighbors.alive) > -1) {
          state = 'alive';
        }
        ...
        if (cell.get('state') === 'zombie') {
          ...
          if (neighbors.alive > 6) state = 'dead';
        }
        evolvedCell.set({
          state: state,
